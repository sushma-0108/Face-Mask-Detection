...
def load_tflite_model():
    interpreter = tf.lite.Interpreter(model_path="face_mask_detector.tflite")
...
if not model_loaded:
    st.error("⚠️ Model file `face_mask_detector.tflite` not found. Place it in the same folder as `app.py`.")
    st.stop()
...
if uploaded_file:
    ...
    is_mask = pred_class == 0  # 0 = with_mask
...
<div style="text-align:center;padding:2.5rem 0 0;
    ...
    MaskGuard AI · Built with TensorFlow & Streamlit
</div>
""", unsafe_allow_html=True)