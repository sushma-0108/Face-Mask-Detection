...
@st.cache_resource
def load_model():
    interpreter = tflite.Interpreter(model_path="face_mask_detector.tflite")
...
if not model_loaded:
    st.error(f"⚠️ Could not load model. Make sure `face_mask_detector.tflite` is in the same folder.")
    st.stop()
